...
// damp movement by propotion
void v3dampMM(vec3& u, const vec3& v)
{
	u.x *= (1 - v.x); 
	u.y *= (1 - v.y);
	u.z *= (1 - v.z);
...
{
	if (u.x < 0 && u.x < -v.x){u.x += v.x;}
	else if(u.x > 0 && u.x > v.x){u.x -= v.x; }
	else{u.x = 0;}
	
	if (u.y < 0 && u.y < -v.y) { u.y += v.y; }
	else if (u.y > 0 && u.y > v.y) { u.y -= v.y; }
	else { u.y = 0; }

	if (u.z < 0 && u.z < -v.z) { u.z += v.z; }
	else if (u.z > 0 && u.z > v.z) { u.z -= v.z; }
	else { u.z = 0; }
}
...
//clips u to v
void v4ClipMinM(vec4 u, const vec4 v)
{
	if (u[0] < v[0]) u[0] = v[0];
	if (u[1] < v[1]) u[1] = v[1];
	if (u[2] < v[2]) u[2] = v[2];
...
void multi(const vec4& u, const float v[16], vec4& retV)
{
	vec4 temp; 
	//v4FloatToVecM(temp, v[0], v[1], v[2], v[3]); 
	//retV[0] = dot(u, temp); 
	//
	//v4FloatToVecM(temp, v[4], v[5], v[6], v[7]);
	//retV[1] = dot(u, temp); 

	//v4FloatToVecM(temp, v[8], v[9], v[10], v[11]);
	//retV[2] = dot(u, temp);

	//v4FloatToVecM(temp, v[12], v[13], v[14], v[15]);
	//retV[3] = dot(u, temp);

	v4FloatToVecM(temp, v[0], v[4], v[8], v[12]);
	retV[0] = dot(u, temp);

	v4FloatToVecM(temp, v[1], v[5], v[9], v[13]);
	retV[1] = dot(u, temp);

	v4FloatToVecM(temp, v[2], v[6], v[10], v[14]);
	retV[2] = dot(u, temp);

	v4FloatToVecM(temp, v[3], v[7], v[11], v[15]);
	retV[3] = dot(u, temp);
