...
char* findFile(char *fname)
{
   ...
   fileFound = (char *)(calloc(strlen(fname)+15, sizeof(char)));
...
int readHeightField (char *filename, int *width, int *depth, float heights[][MAXDEPTH])
...
#define giveUp(message) {fclose(inputFile); printf("%s\n", message); return GL_FALSE;}
{
  ...
  for (j=0; j<*depth; j++)
  {
    for (i=0; i<*width; i++)
    {
      nRead = fscanf_s(inputFile, "%f", &heights[i][j]);
      if (nRead != 1)
...
int getBMPTrans(char *fname, GLuint* handle)
{
	FILE *fp;
	...
	printf("  Binding texture to handle: %d\n", *handle);

	int temp;
	for (int i = 0; i < (*bmpData.BitmapInfo).bmiHeader.biSizeImage; i += 4)
	{
		temp = (unsigned char)(bmpData.BitmapBits[i] + (unsigned char)bmpData.BitmapBits[i + 1] + (unsigned char)bmpData.BitmapBits[i + 2]);
		//unsigned char temp = (bmpData.BitmapBits[i + 3]); 
		temp = min(temp, 255);
		temp = temp < 20 ? 0 : temp;
		bmpData.BitmapBits[i + 3] = temp;
	}
	...
	fclose(fp);
	SaveDIBitmap("../textures/new_laser.bmp", bmpData.BitmapInfo, bmpData.BitmapBits);
...
int getBMPTexture(char *fname, GLuint* handle, GLboolean buildMipmaps, GLenum src_format, GLenum des_format)
...
// or -1 if read failed.
{
  ...
  if (buildMipmaps)
  {
		gluBuild2DMipmaps(
			GL_TEXTURE_2D,       // target type of texture
			src_format,              // format in which GL should store it
			texWidth, texHeight, // [obvious]
			des_format,          // format of data
			GL_UNSIGNED_BYTE,    // each R,G,B component is a byte
  ...
  else
  {
		glTexImage2D(
			GL_TEXTURE_2D,       // target type of texture
			0,                   // mipmap level
			src_format,              // format in which GL should store texels
			texWidth, texHeight, // [obvious]
			0,                   // border width
			des_format,          // format of data
			GL_UNSIGNED_BYTE,    // each R,G,B component is a byte
